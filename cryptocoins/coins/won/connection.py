...
WON_PROVIDERS_CACHE = 'WON_PROVIDERS_CACHE'
WON_RESPONSE_TIME_COUNTER_CACHE = 'WON_RESPONSE_TIME_COUNTER'


log = logging.getLogger(__name__)

def set_endpoints(endpoints: deque):
    cache.set(WON_PROVIDERS_CACHE, endpoints)


def get_current_endpoint():
    endpoints = cache.get(WON_PROVIDERS_CACHE)
    if not endpoints:
        endpoints = deque(settings.WON_RPC_ENDPOINTS)
...
def check_won_response_time(w3, time_sec):
    current_counter = cache.get(WON_RESPONSE_TIME_COUNTER_CACHE, 0)
    if time_sec >= 2.8:
        current_counter += 1
    else:
        current_counter = 0
    ...
    cache.set(WON_RESPONSE_TIME_COUNTER_CACHE, current_counter)
